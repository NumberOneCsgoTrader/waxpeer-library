...
export class TradeWebsocket extends EventEmitter {
    ...
    constructor(apiKey: string, steamid: string, tradelink: string, waxApi?: string, accessToken?: string) {
        ...
        if (!this.apiKey && !this.waxApi && !this.accessToken) throw new Error("Steam API or Waxpeer API or access token is required");
        this.connectWss();
    }
    socketOpen() {
    ...
    async connectWss() {
        ...
        this.w.ws.on("close", (e) => {
            ...
            setTimeout(
                function () {
                    if (this.steamid && this.apiKey && this.allowReconnect && this.w?.ws?.readyState !== this.readyStatesMap.OPEN) {
        ...
        this.w.ws.on("open", (e) => {
            ...
            if (this.steamid) {
                ...
                this.w.int = setInterval(() => {
                    if (this.w?.ws && this.w.ws.readyState === this.readyStatesMap.OPEN) this.w.ws.send(JSON.stringify({ name: "ping" }));
                }, 25000);
            } else {
                this.w.ws.close();
            }
